...
def read_water_tiles_only(tile_size):
    # water_tiles_file = ("water_tiles_list_1440.txt")
    water_tiles_file = (f"water_tiles_list_{tile_size}.txt")
    ...
    try:
        ...
        #base_path = os.path.expanduser(f"~/drone_processing/data/chips_{tile_size}/")
        base_path = os.path.expanduser(f"~/drone_processing/data/chips_{tile_size}/")
...
def read_all_tiles(tile_size):
    print(f"~/drone_processing/data/chips_{tile_size}/")
    base_path = os.path.expanduser(f"~/drone_processing/data/chips_{tile_size}/")
...
def test_on_orthophoto(net, tile_size):
    ...
    test_images, water_tiles_names = read_all_tiles(tile_size)
    ...
    for filename, img in zip(water_tiles_names, test_images):
        ...
        cax2 = ax2.imshow(pred*norm_param_depth)
        ...
        fig.colorbar(cax2, ax=[ax1, ax2], orientation='vertical', label='Depth Values', fraction=0.02, pad=0.05)
        plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
        fig.savefig(f"soca/uav_{location}/tiles_{tile_size}/test_image_result_{filename[:-4]}.png")
        plt.close()
...
def main():
    ...
    test_on_orthophoto(net, tile_size=2880)
